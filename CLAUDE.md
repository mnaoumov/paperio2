...
**Module split: in progress.** The single `main()` IIFE that once held the whole engine in `app2.ts` is being extracted into layered ES modules. Done so far: `src/shared/` (`constants`, `ids`, `math-utils`, `color-utils`, `random`, `types` — leaf utilities), `src/geometry.ts` (Vector/Segment/Polyline/Polygon/ContourPoints/SpatialGrid/Border + geometry helpers), `src/i18n.ts` (Language types, `russianLanguage`, `list3`, `buildLanguageList`/`findDefaultLanguage` — a true leaf, imports nothing), `src/ui.ts` (all preact components + `LanguageContext` + storage-seam types), and `src/engine.ts` (the remaining coupled core: entities, particles, scoring, skins, Game, Controller, render, and `createGameApi`). `src/app2.ts` is now a thin entry: the `declare global` block plus the bootstrap, importing from `engine.ts`/`ui.ts`/`i18n.ts`. Still inside `engine.ts` (candidates for further peeling): skins, entities, particles, scoring, and Game/controller/render/api.

- **No circular dependencies — enforced by `import-x/no-cycle` (already enabled).** The engine is one *type-level* strongly-connected component, so lower modules reference higher-layer types (e.g. `geometry` `import type`s `Trail`/`ShapeOwner` from `engine`). Those back-edges are **type-only**, erased at build, and `import-x/no-cycle` ignores them (verified: it flags value cycles, not `import type` cycles). The **value**-import graph is a strict DAG: `shared` → `geometry` → `engine` → `ui` → `app2`, with `i18n` a leaf below everything. Rule when peeling a module: value-import only downward; use `import type` for any reference to a higher layer; if two classes genuinely value-cycle, co-locate them.
