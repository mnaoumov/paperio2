...
**Module split: complete.** The single `main()` IIFE that once held the whole engine in `app2.ts` (7462 lines) has been fully extracted into 12 layered ES modules; the entry is now **`src/main.ts`** (~90 lines): the bootstrap, run via `main().catch(console.error)` because esbuild's `iife` output format rejects top-level `await`; the ambient `window`/DOM augmentations moved to **`src/global.d.ts`** (a module `.d.ts`, `export {}` so `declare global` augments). The esbuild entry points at `src/main.ts` but the output is still **`dist/app2.js`** (unchanged `outfile`). Modules: `src/shared/` (`constants`, `ids`, `math-utils`, `color-utils`, `random`, `types`, `noop` — leaf utilities); `src/geometry.ts` (Vector/Segment/Polyline/Polygon/ContourPoints/SpatialGrid/Border + geometry helpers + `angleToVector`); `src/i18n.ts` (Language types, `russianLanguage`, `list3`, `buildLanguageList`/`findDefaultLanguage` — a true leaf, imports nothing); `src/ui.ts` (all preact components + `LanguageContext` + storage-seam types); `src/skins.ts` (SkinLayer/Asset/SkinManager… + `loadImage`, `createColorTile`, `COLOR_PALETTE`); `src/scoring.ts` (SchemeCycler/ScoreLabel/Achievement… + `readCookieJson`); `src/particles.ts` (Particle/TextParticle + `spawnScoreParticles`); `src/entities.ts` (Territory/Trail/StateMachine + bot-AI, City/Unit/Player/Bot/NamePool + entity types); `src/controller.ts` (KeyboardModeSwitch/Controller + input state types + keycode constants); `src/game-render.ts` (the canvas render layer — `renderGame` + all `draw*`/`render*` helpers + render constants + RenderContext + the crown/face path caches); `src/game-api.ts` (`createGameApi`/`GameApi` + the debug-combo wiring). `src/engine.ts` (~1940 lines) is the remaining **Game core**: the `Game` class, `Config`/`defaultConfig`, `GameResults`, the domain-lock check, and the shared game types/constants. Render + api peeled together because `createGameApi` wires `game.renderer = renderGame` (splitting only render would form an `engine ↔ game-render` value cycle); the value DAG is `engine → game-render → game-api` since `Game` never value-calls the render layer or the API.
...
- A readable, typed **reconstruction** of the engine lives in the **`ts/` project**
  ...
  subsection above** for the current file layout (`shared/`, `geometry.ts`, `i18n.ts`, `ui.ts`,
  `skins.ts`, `scoring.ts`, `particles.ts`, `entities.ts`, `controller.ts`, `game-render.ts`,
