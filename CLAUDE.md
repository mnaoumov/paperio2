...
**Module split: in progress.** The single `main()` IIFE that once held the whole engine in `app2.ts` (7462 lines) has been extracted into layered ES modules. `app2.ts` is now a **181-line thin entry** (the `declare global` block + the bootstrap). Extracted modules: `src/shared/` (`constants`, `ids`, `math-utils`, `color-utils`, `random`, `types`, `noop` — leaf utilities); `src/geometry.ts` (Vector/Segment/Polyline/Polygon/ContourPoints/SpatialGrid/Border + geometry helpers + `angleToVector`); `src/i18n.ts` (Language types, `russianLanguage`, `list3`, `buildLanguageList`/`findDefaultLanguage` — a true leaf, imports nothing); `src/ui.ts` (all preact components + `LanguageContext` + storage-seam types); `src/skins.ts` (SkinLayer/Asset/SkinManager… + `loadImage`, `createColorTile`, `COLOR_PALETTE`); `src/scoring.ts` (SchemeCycler/ScoreLabel/Achievement… + `readCookieJson`); `src/particles.ts` (Particle/TextParticle + `spawnScoreParticles`); `src/entities.ts` (Territory/Trail/StateMachine + bot-AI, City/Unit/Player/Bot/NamePool + entity types); `src/controller.ts` (KeyboardModeSwitch/Controller + input state types + keycode constants). `src/engine.ts` (~2960 lines) is now the **Game core**: the `Game` class, the canvas render layer (`renderGame` + `draw*` helpers), `createGameApi`/`GameApi`, `Config`/`defaultConfig`, the domain-lock check, and the shared game types/constants. The render layer + `createGameApi` are the last still-coupled pieces (they'd peel together — `createGameApi` wires `game.renderer = renderGame`, so splitting only render would create an `engine ↔ render` value cycle).

- **No circular dependencies — enforced by `import-x/no-cycle` (already enabled).** The engine is one *type-level* strongly-connected component, so lower modules reference higher-layer types (e.g. `geometry` `import type`s `Trail`/`ShapeOwner` from `engine`). Those back-edges are **type-only**, erased at build, and `import-x/no-cycle` ignores them (verified: it flags value cycles, not `import type` cycles). The **value**-import graph is a strict DAG: `shared`/`i18n` (leaves) → `geometry` → `controller`/`particles`/`scoring`/`skins`/`entities` → `engine` (Game core) → `ui` (via `i18n`) → `app2`. Every module value-imports only downward and reaches higher-layer classes via `import type`; e.g. `engine` value-imports `Bot`/`Player`/`City` from `entities` while `entities` references `Game` only as a type. Rule when peeling a module: value-import only downward; use `import type` for any reference to a higher layer; if two classes genuinely value-cycle, co-locate them; and move any constant/helper used by both the peeled module and a *lower* layer into `shared`/`geometry` (e.g. `KILL_REASON_*`, `MILLISECONDS_IN_SECOND`, the angle constants, `angleToVector`, `noop`) so the import stays downward.
...
- A readable, typed **reconstruction** of the engine lives in the **`ts/` project**
  (`ts/src/`, entry `ts/src/app2.ts`). It is the single maintained deobfuscated source (the
  older root-level `app2.deobfuscated.js`/`.ts` copies have been removed). Originally one big
  IIFE in `app2.ts`, it is being split into layered ES modules — see **the module-split
  subsection above** for the current file layout (`shared/`, `geometry.ts`, `i18n.ts`,
  `ui.ts`, `engine.ts`) and the no-circular-dependency / lint conventions. The engine was originally shipped
