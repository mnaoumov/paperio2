 ...
 *
 * This repo resolves no dependency on `obsidian-dev-utils` - it is scaffolded from `typescript-template`, not
 * a consumer of that library - so the custom rules it lints itself with are hand-copied out of that repo's
 ...
 *    `package.json`. Walking one known path per repo is how a sibling's second tree was never compared to
 *    anything.
 * 3. **This repo's side is READ FROM THE INDEX**, not from the working tree ({@link readVendoredText}). A
 ...
 *
 * This runs from `nano-staged` on a staged rule source, and also from CI. The CI half is what catches the
 * drift direction the pre-commit half cannot: a copy is only staged when somebody is editing it, so upstream
 * moving underneath a tree nobody touches is invisible to a hook. Here CI is the half that actually fires:
 * `ts/` is a subfolder of the paperio2 preservation repo, so husky is deliberately not wired up (AGENTS.md
 * says why) and the `nano-staged` entry only runs when `nano-staged` is invoked by hand. The CI step is
 * therefore load-bearing in this repo rather than a backstop, which is the reverse of the sibling repos this
 * gate was copied from.
 *
 * Offline, or anywhere the fetch is unwelcome, this is turned off with `CHECK_VENDORED_ESLINT_RULES=0`, via
 * {@link exitIfScriptDisabled} - the per-script off switch the sibling repos give every script, brought over
 * with this gate, which is the first script here to carry one. That is an explicit opt-out rather than a
...
/*
 * This repo's reading of `require-method-template.ts`'s named groups, and the import that serves it. See
 * {@link UPSTREAM_NAMED_GROUP_READS} for why it is not upstream's.
 */
const LOCAL_NAMED_GROUP_READS = `    const groups = ensureNonNullable(match.groups);
    const name = ensureNonNullable(groups['typeName']);
    const rest = ensureNonNullable(groups['rest']).trim();`;

const LOCAL_TYPE_GUARDS_IMPORT = `/*
...
/*
 * Upstream's two reads of `require-method-template.ts`'s named groups, and the import that serves them. Both
 * are matched as literal text: when upstream rewrites either, the replacement no-ops and the file is reported
 * as drift, which is the right answer - this arm records a divergence from a KNOWN upstream shape, and a
 * shape that has moved is exactly what wants a human's eyes.
 */
const UPSTREAM_NAMED_GROUP_READS = `    const name = getMandatoryNamedGroup(match, 'typeName');
    const rest = getMandatoryNamedGroup(match, 'rest').trim();`;

const UPSTREAM_REG_EXP_IMPORT = 'import { getMandatoryNamedGroup } from \'../../../reg-exp.ts\';';
 ...
 * @param upstreamFileNames - The names that identify a copy, as upstream publishes them.
 * @returns Every matching path, sorted, so a run's report reads the same way twice.
 */
...
/**
 * Compares one vendored copy against upstream's bytes after the recorded transform, recording a failure when
 * they differ.
 *
 * @param vendoredPath - The copy, posix-spelled and absolute.
 * @param root - The repository root, which the reported path is relative to.
...
/**
 * Reads one rule source from upstream.
 *
 * @param fileName - The source's file name, as the upstream listing spells it.
 * @returns Upstream's text, or `null` when upstream no longer publishes that name.
 */
 ...
 *
 * @returns Every name upstream publishes in that directory.
 */
 ...
 *
 * @param upstreamText - Upstream's bytes.
 * @param fileName - The file's name, which decides which arms claim it.
