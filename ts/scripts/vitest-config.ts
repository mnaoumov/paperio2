    ...
    projects: [
      ...
      },
      {
        test: {
           ...
           * an upgrade. It stayed invisible here because this project was the only one contributing specs:
           * `src/` carries no test file, so until `unit-tests:scripts` below arrived there was never a
           * second project in group 0 to collide with. The delta belongs on this project rather than on
           * its siblings, since this is the one asking for the non-default worker count.
           */
      ...
      },
      {
         ...
         * nothing. `scripts/helpers/eslint.test.ts` is the first such file and is the reason this project
         * exists. Its shape (name included) is the one `typescript-template` uses, so the two configs stay
         * comparable.
         *
