 ...
 *
 * An npm script that calls {@link exitIfScriptDisabled} can be skipped by setting an environment variable
 * named after it - the script name uppercased, with every non-alphanumeric character replaced by `_`:
 *
 * | Script         | Switch           |
 * |----------------|------------------|
 * | `lint`         | `LINT=0`         |
 ...
 * `scripts/nano-staged-config.ts` reads its own switch through {@link isEnvVariableOff} here.
 *
 * This file arrived with `check:vendored-eslint-rules`, today the only script here that calls
 * {@link exitIfScriptDisabled}; in the sibling repos it is hand-synced with, every `scripts/*.ts` entry
 * point calls it.
 */
 ...
 *
 * Called as the first statement of a `scripts/*.ts` entry point, so a script never has to declare its own
 * switch, and each `npm run` sub-step of a composite script carries its own.
 */
