...
- **husky is present but intentionally NOT wired.** `ts/` is a subfolder of the paperio2 git repo (root `F:\dev\projects\paperio2`), so activating husky (setting `core.hooksPath`) would hijack the whole preservation repo's commit hooks. The `.husky/` files are kept as faithful copies but there is **no `prepare` script** — run the gate manually before committing. The pre-commit tasks (`.nano-staged.mjs`) operate on staged files only.
- **The Node pin: `ts/.nvmrc` = 26, `engines.node` = `>=22.0.0` — deliberately different numbers.** `.nvmrc` is the version to DEVELOP on, matching the `typescript-template` reference this tooling was ported from, and this machine’s default Node; `engines` is the floor the code actually needs. They are kept apart on purpose, and `@types/node` (`^26.1.1`) follows `.nvmrc` rather than `engines` — the Node scripts are type-checked against 26’s API surface, so running them on an older major is the drift, not the safety margin. nvm-windows auto-switches on `.nvmrc`, so **every** command issued from `ts/` — `node`, `npm`, and therefore `typecheck`, `lint`, `format:check`, `spellcheck`, `lint:md`, `test`, `build` — runs under that major; if it is not installed on this machine all of them die with `Node.js vXX.x.x is not installed or cannot be found`, and the fix is `nvm install 26`, never an edit here. CI reads the same file (`node-version-file: ts/.nvmrc`), so the pin is changed in exactly one place. Verified green under 26.5.0 on 2026-09-20: `npm ci` plus all eight gates, `check:vendored-eslint-rules` and 141 passing tests among them.
- **The one Node 26 incompatibility in this repo does NOT reach `ts/`, and must not be read as an argument for lowering the pin.** `research/deob/` needs webcrack, whose `isolated-vm` dependency fails its native build on 26, which is why `research/deob/NOTES.md` opens its pipeline with `nvm use 24.18.0`. That directory is git-ignored, is a one-time toolchain rather than part of the build, and sits outside `ts/` — so `ts/.nvmrc` neither governs it nor is constrained by it, and the two version numbers are independent by design.
...
- **The vendored ESLint rules are gated against upstream.** `npm run check:vendored-eslint-rules` (`ts/scripts/check-vendored-eslint-rules.ts`, copied from the template) asserts that every file under `ts/scripts/helpers/eslint-rules/` is byte-identical to `obsidian-dev-utils`' published source after three recorded transform arms. It walks for the FILE NAME rather than a fixed directory, reads this side out of the **git index** (`ts/scripts/helpers/git-content.ts`) so a concurrent `lint:fix` cannot race it, and lists upstream's files from the GitHub contents API. Because husky is not wired up here (above), the `.nano-staged.mjs` entry never fires on its own — **the CI step is the half that actually runs**, so do not drop it. Never hand-edit a vendored copy: take upstream whole, or record the delta as a transform arm. `CHECK_VENDORED_ESLINT_RULES=0` turns it off for an offline run.
- **`npm run lint` tells a crashed linter apart from a red codebase, and that distinction is load-bearing here.** The ESLint child on this machine dies with exit `3221225477` (`0xC0000005`, an access violation) and no output of its own — **measured at 1 run in 30** on an unchanged, lint-clean tree — and npm reports that to *its* caller as a plain exit `1`, which is indistinguishable from "the code is red". So `ts/scripts/helpers/eslint.ts` no longer lets `execFromRoot` throw on the exit code: it captures the result and classifies it (`classifyEslintExit`) as `clean` / `lint-problems` (exit `1`, the only code that is evidence about the SOURCE) / `did-not-lint` (everything else). A `did-not-lint` throws `describeFailureToLint`'s message, which names which of the four happened — killed by a signal, died at the OS level, a rule threw (`Rule: "…"` in stderr), or ESLint aborted on a broken config or command line — quotes back what ESLint said minus the banner, npm's chatter and the stack frames, and says outright that the source has not been judged. **The `Oops! Something went wrong!` banner is NOT a crash marker** — ESLint prints it for a mistyped path too, which is why the rule crash is separated out by the `Rule:` line instead. Covered by `ts/scripts/helpers/eslint.test.ts` against verbatim stderr from both, including the OS-level death that cannot be triggered on demand. **If CI or a local gate ever reports a lint failure, read the message before reading the source** — and note that `lint:fix` goes through the same helper.
  - Two things had to move for it. `executeBatches` in `ts/scripts/helpers/exec.ts` returned a hard-coded `exitCode: 0` when asked for details, so a batched command (which is how `lint:fix` runs under nano-staged) reported success however its batches ended; it now carries the first failing batch's code and signal. And `ts/scripts/vitest-config.ts` gained a third project, `unit-tests:scripts` — `unit-tests` excludes `scripts/**` and the `eslint-rules` project takes only that folder, so until now a test anywhere else under `scripts/` **ran nowhere** while `npm test` stayed green. Its shape (and the `sequence.groupOrder` that keeps `maxWorkers: 1` off the default group) is the `typescript-template` one; the comments there explain why.
